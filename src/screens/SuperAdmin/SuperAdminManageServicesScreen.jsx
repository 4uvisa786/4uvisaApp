...
const FIELD_TYPES = ['text', 'number', 'phone', 'date', 'select', 'file', 'dropdown'];
...
export default function AdminManageServicesScreen() {
  ...
  // Inside AdminManageServicesScreen
const [editingFieldIdx, setEditingFieldIdx] = useState(null);
  ...
  const startEditFormField = (field, index) => {
  ...
  const newField = {
    ...
    options: (fieldInput.type === 'select' || fieldInput.type === 'dropdown') ? [...(fieldInput.options || [])] : [],
  };
  ...
  setCurrentSubService(prev => {
    ...
    if (editingFieldIdx !== null) {
      // Logic for UPDATING
      updatedFields[editingFieldIdx] = newField;
    } else {
      // Logic for NEW (with unique check)
      if (updatedFields.find(f => f.name === fieldInput.name)) {
        Alert.alert('Validation Error', 'Form field name must be unique.');
        return prev;
      }
  ...
  setFieldInput({ label: '', name: '', type: 'text', required: false, options: [], optionInput: '' });
};
  ...
  <TouchableOpacity 
    key={idx} 
    onPress={() => startEditFormField(f, idx)} // <-- Clicking this now fills the form above
    activeOpacity={0.8}
  >
    <View style={[
      styles.fieldCard, 
      editingFieldIdx === idx && { borderColor: Colors.primary, borderWidth: 1 } // Visual highlight for editing
    ]}>
      <View style={styles.fieldCardHeader}>
        <View style={styles.fieldCardLeft}>
          <MaterialIcons 
            name={
              ...
              f.type === 'dropdown' ? 'arrow-drop-down-circle' :
              'attach-file'
            } 
            size={18} 
          ...
          <View>
            <Text style={styles.fieldLabel}>
              {f.label} {editingFieldIdx === idx ? "(Editing...)" : ""}
            </Text>
            <Text style={styles.fieldMeta}>
        ...
        {/* Delete Button (keep separate from the Edit click) */}
        <TouchableOpacity 
          onPress={(e) => {
            e.stopPropagation(); // Prevents triggering the 'Edit' click when deleting
            removeFormField(idx);
          ...
          {(f.options || []).map((op, i) => (
            <Text key={i} style={styles.optionText}>• {op}</Text>
                  ...
                  <View style={styles.addFieldSection}>
                    ...
                    {(fieldInput.type === 'select' || fieldInput.type === 'dropdown') && (
                    ...
                    {/* === PASTE THE CODE HERE === */}
    <TouchableOpacity
      style={[styles.addFieldBtn, editingFieldIdx !== null && { backgroundColor: Colors.primary }]}
      onPress={addFormField}
      activeOpacity={0.8}
    >
      <MaterialIcons 
        name={editingFieldIdx !== null ? "check-circle" : "add-circle"} 
        size={20} 
        color={Colors.textLight} 
      />
      <Text style={styles.addFieldBtnText}>
        {editingFieldIdx !== null ? "Update Field" : "Add Field to Form"}
      </Text>
    </TouchableOpacity>
    ...
    {editingFieldIdx !== null && (
      ...
      >
        <Text style={{ color: Colors.error, fontWeight: '600' }}>Cancel Edit</Text>
      </TouchableOpacity>
    )}
    {/* === END OF PASTE === */}
